...
class ProfileDesignViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let flowLayout = collectionView.collectionViewLayout as? UICollectionViewFlowLayout {
            flowLayout.minimumLineSpacing = 2 // 셀 사이의 수직 간격
            flowLayout.minimumInteritemSpacing = 2 // 셀 사이의 수평 간격
            let itemWidth = (view.frame.width - 4) / 3 // 화면을 3등분하여 셀의 너비 계산
            flowLayout.itemSize = CGSize(width: itemWidth, height: itemWidth) // 셀의 크기 설정
        }
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(navBarImageViewTapped))
        navBarImageView.isUserInteractionEnabled = true // 사용자 상호 작용을 가능하게 함
        navBarImageView.addGestureRecognizer(tapGesture)
        ...
        NSLayoutConstraint.activate([
            ...
            navBar.topAnchor.constraint(equalTo: separatorView.bottomAnchor, constant: 420),
//                navBar.heightAnchor.constraint(equalToConstant: 100),
            ...
            collectionView.topAnchor.constraint(equalTo: separatorView.bottomAnchor, constant: 41),
    ...
    @objc func navBarImageViewTapped() {
...
extension ProfileDesignViewController: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        // 컬렉션뷰의 총 아이템 수를 반환 (더미 이미지 수에 따라 수정)
        return 10 // 예시로 9개의 더미 이미지가 있다고 가정
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        // 더미 이미지 파일 이름을 설정
        let imageName = "\(indexPath.item + 1)" // 이미지 파일 이름은 1, 2, 3, 4, 5로 순차적으로 설정됨
        
        // 더미 이미지 설정 (이미지 파일 이름이 Assets에 있는 이미지와 일치해야 함)
        ...
        cell.contentView.backgroundColor = UIColor.gray // 셀 배경색 설정 (선택 사항)
